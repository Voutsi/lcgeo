...
//--------------------------------------------------------------------
// Modification of Beampipe_o1_v01_geo.cpp in order to accommodate a gold coating at inner part of the Be beam pipe
//  Y. Voutsinas
//  $Id: Beampipe_o1_v01_geo.cpp 906 2016-02-12 09:47:40Z gaede $
//====================================================================
...
class SimpleCylinderImpl : public VolCylinderImpl{
  double _half_length ;
public:
...
public:
  SimpleCylinder( Volume vol, SurfaceType type, double thickness_inner ,
...
static Ref_t create_element(Detector& theDetector,
  ...
  for(xml_coll_t c( xmlBeampipe ,Unicode("section")); c; ++c) {
    ...
    const std::string Name      = xmlSection.nameStr();
    ...
    Material goldMaterial    = theDetector.material("Gold"); // it supposed to be a gold coating so we can hard-coded the gold as material
	...
	// the gold coating: another tubular cone
	ConeSegment goldSolid( zHalf, rInnerStart-(goldWidth+goldTolerance), rInnerStart-goldTolerance, rInnerEnd-(goldWidth+goldTolerance), rInnerEnd-goldTolerance, phi1, phi2);
	Volume goldLog ( volName + "_gold", goldSolid, goldMaterial);
	Volume goldLog2( volName + "_gold2", goldSolid, goldMaterial);
	...
	if( crossType == ODH::kCenter ) { 
	  ...
	  if (isCylinder) {  // cylinder
	    ...
	    if (Name=="FirstConeAu"){
	      ...
	      VolCone auconSurf1( goldLog , SurfaceType( SurfaceType::Helper ) , 0.5*goldWidth  , 0.5*goldWidth , auv, aucon );
	      VolCone auconSurf2( goldLog2, SurfaceType( SurfaceType::Helper ) , 0.5*goldWidth  , 0.5*goldWidth , auv, aucon );

	      volSurfaceList( tube )->push_back( auconSurf1 );
	      volSurfaceList( tube )->push_back( auconSurf2 );
	    }
	    //----------------------------------------------------------------------------------------
	...
	goldLog.setVisAttributes(theDetector, "GoldVis");
	goldLog2.setVisAttributes(theDetector, "GoldVis");
    ...
    case ODH::kPunchedDnstream: {
      ...
      // solid for the tube (including vacuum and wall): a solid cone
      ConeSegment tubeSolid( zHalf, 0, rOuterStart, 0, rOuterEnd, phi1, phi2);
      //ConeSegment tubeSolid( zHalf, 0, 14*dd4hep::mm, 0, 14*dd4hep::mm, phi1, phi2);  
...
DECLARE_DETELEMENT(Beampipe_o2_v02,create_element)
