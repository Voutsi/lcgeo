...
static DD4hep::Geometry::Ref_t create_element(DD4hep::Geometry::LCDD& lcdd,
  ...
  for(xml_coll_t c( xmlBeampipe ,Unicode("section")); c; ++c) {
    ...
    Material goldMaterial    = lcdd.material("Gold"); // it supposed to be a gold coating so we can hard-coded the gold as material
	...
	goldLog.setVisAttributes(lcdd, "GoldVis");
	goldLog2.setVisAttributes(lcdd, "GoldVis");
